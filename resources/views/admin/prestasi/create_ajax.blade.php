...
<script>
    $(document).ready(function () {
        // Inisialisasi Select2 pada form
        $('.select2').select2({
            theme: 'bootstrap4',
            dropdownParent: $('#modal-master')  // Pastikan dropdownParent mengarah ke modal yang benar
        });

        // Inisialisasi Select2 untuk mahasiswa dengan pencarian AJAX
        $('#mahasiswa_id').select2({
            placeholder: 'Cari Mahasiswa...',
            multiple: true,
            ajax: {
                url: "{{ url('/prestasi/mahasiswa/search') }}",
                dataType: 'json',
                delay: 250,
                ...
                processResults: function (data) {
                    return {
                        results: data.map(function (item) {
                            return { id: item.nim, text: item.nama_lengkap };
                        })
                    };
                },
                cache: true
